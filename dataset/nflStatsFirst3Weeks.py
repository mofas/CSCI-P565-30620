...
header = ['Name', 'Position', 'Team', 'Passing_Yards','Rushing_Yards','Receiving_Yards',	'Passing_TDs',	'Rushing_TDs',	
          ...
          'Forced_Fumbles','Sacks','Blocked_Kicks','Blocked_Punts','Safeties','Kickoff_Return_TD','Punt_Return_TD','Defensive_TD']
...
with open('nfl_stats_first3.csv', 'w') as fp:
    ...
    for p in players:
        wr.writerow([p,p.guess_position, p.team,p.passing_yds,p.rushing_yds, p.receiving_yds, p.passing_tds,p.rushing_tds, 
                     p.receiving_tds, p.kicking_fgm, p.kicking_fga - p.kicking_fgm, p.kicking_xpmade, p.passing_int,  p.fumbles_lost, p.defense_int, p.defense_ffum, p.defense_sk, p.defense_xpblk+p.defense_fgblk, p.defense_puntblk, p.defense_safe, p.kickret_tds, p.puntret_tds, p.defense_tds])
